...
my $WIDTH = 20;
my $HEIGHT = 20;
my $cmd = 'opencv_createsamples -bgcolor 0 -bgthresh 0 -maxxangle 1.1 -maxyangle 1.1 maxzangle 0.5 -maxidev 40 -w $WIDTH -h $HEIGHT';
my $totalnum = 7000;
my $tmpfile  = 'tmp';
...
#for (my $k = 0; $k < 1; $k++ ) {
    ...
    my $vec = $outputdir . substr($tokens[0], $imgdirlen) . ".vec" ;    
    print "==========> splitted image: " . $tokens[0] . "\n";
    print "==========> vec file: " . $vec . "\n";
    print "$cmd -img $tokens[0] -bg $tmpfile -vec $vec -num $num" . "\n";
    system("$cmd -img $tokens[0] -bg $tmpfile -vec $vec -num $num");
}
unlink($tmpfile);
