...
# Date  : 03/12/2006
#########################################################################
my $cmd = 'opencv_createsamples -bgcolor 0 -bgthresh 0 -maxxangle 1.1 -maxyangle 1.1 -maxzangle 0.5 maxidev 40';
...
for (my $k = 0; $k < $#positives; $k++ ) {
    ...
    my @tokens = split(/\s+/, $img);
    
    my $subbasename = substr($tokens[0], $imgdirlen);
    my $info = $outputdir . $subbasename . "/info.dat";
    print "$cmd -img $tokens[0] -bg $tmpfile -info $info -num $num" . "\n";
    system("$cmd -img $tokens[0] -bg $tmpfile -info $info -num $num");
